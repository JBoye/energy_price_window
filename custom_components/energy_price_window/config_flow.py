...
DATA_SCHEMA = vol.Schema(
	{
		...
		vol.Optional(CONF_START_TIME, default=""): selector.TextSelector(),  # e.g. {{ today_at('16:00') }} or blank
		vol.Optional(CONF_END_TIME, default=""): selector.TextSelector(),    # e.g. {{ today_at('22:00') }} or blank
		...
		schema = vol.Schema(
			{
				...
				vol.Optional(CONF_START_TIME, default=str(data.get(CONF_START_TIME, "") or "")): selector.TextSelector(),
				vol.Optional(CONF_END_TIME, default=str(data.get(CONF_END_TIME, "") or "")): selector.TextSelector(),
				vol.Required(CONF_DURATION, default=str(data.get(CONF_DURATION, DEFAULT_DURATION))): selector.TextSelector(),
				vol.Optional(CONF_CONTINUOUS, default=bool(data.get(CONF_CONTINUOUS, DEFAULT_CONTINUOUS))): selector.BooleanSelector(),
			}
		)
