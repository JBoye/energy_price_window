...
from .const import (
	ATTR_AVERAGE, ATTR_CONTINUOUS, ATTR_DURATION, ATTR_END_TIME, ATTR_INTERVALS, ATTR_LAST_CALCULATED, ATTR_NEXT_START_TIME, ATTR_START_TIME,
	CONF_CONTINUOUS, CONF_DURATION, CONF_END_TIME, CONF_SOURCE_ENTITY, CONF_FORECAST_SOURCE_ENTITY,
	CONF_START_TIME, CONF_NAME, DEFAULT_NAME, DEFAULT_START_TIME, DEFAULT_END_TIME, DEFAULT_DURATION, DEFAULT_CONTINUOUS,
)
...
class PriceWindowBinarySensor(BinarySensorEntity):
	...
	async def async_added_to_hass(self) -> None:
		...
		def _sub_tmpl(tmpl: template.Template | None):
			if tmpl is None:
				return
			res = async_track_template_result(self.hass, [TrackTemplate(tmpl, None)], self._handle_template_result)
	...
	async def _handle_template_result(self, *_): await self._recalc()
	...
	def _parse_datetime(self, value: Any) -> Optional[datetime]:
		if isinstance(value, datetime):
			return value.replace(tzinfo=dt_util.DEFAULT_TIME_ZONE) if value.tzinfo is None else value
		if isinstance(value, str):
			dt = dt_util.parse_datetime(value)
			if dt:
				return dt.replace(tzinfo=dt_util.DEFAULT_TIME_ZONE) if dt.tzinfo is None else dt
			try:
	...
	def _clip_to_range(self, s: datetime, e: datetime, r0: datetime, r1: datetime) -> Optional[Tuple[datetime, datetime]]:
		...
		s = _loc(s); e = _loc(e); r0 = _loc(r0); r1 = _loc(r1)
		if e <= r0 or s >= r1:
			return None
	...
	async def _recalc(self) -> None:
		...
		self._attr_extra_state_attributes = {
			...
			ATTR_DURATION: duration_td.total_seconds() / 3600,
			ATTR_CONTINUOUS: bool(continuous),
			ATTR_NEXT_START_TIME: dt_util.as_local(next_start).isoformat() if next_start else None,
